...
At the end of an implemented live run, inspect the generated root README and JSON, the public/private data boundary and the documented private Dashboard handoff; report the counts and failures required by the repository instructions. Review Git diffs before an authorized development publication to `origin/dev`. Never update `main` without the user's explicit merge request. At the present stage, only the local schema/renderer and historical preview exist; a live refresh cannot be claimed complete.
