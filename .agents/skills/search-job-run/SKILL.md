...
After a script failure, leave the company in `ai_pending` on `var/review-dashboard.html`; this is a queue, not evidence that AI has already tried. When AI begins one bounded investigation, record `--ai-review-lead KEY --ai-review-outcome investigating --review-note "..."`. If the actual AI attempt still cannot establish official company or board evidence, record `--ai-review-outcome needs-user` only when the user can supply a specific missing link or fact; include that request in the note. An adapter implementation or provider API failure remains agent-owned technical work, not manual review. Open the refreshed Dashboard and tell the user which records actually need their input. Do not mark the whole failed script batch `needs-user` without per-company AI work. If the route is resolved, use `--confirm-route` or retry the corrected script; successful scans leave the unresolved queue. Preserve original Simplify sample and company-profile links in the Dashboard even when the current ATS changed.
