...
A successful handoff records the company, provider, board URL/token, official evidence, brand scope, verification time and bounded-read result, then sends the verified route to [job search](../job-search/SKILL.md). If the board is unreadable because the adapter is broken, hand that defect to [ATS adapter](../ats-adapter/SKILL.md). If official evidence remains insufficient after the bounded AI attempt, keep the mapping unverified and record the exact blocker. Use `--ai-review-lead KEY --ai-review-outcome needs-user --review-note "..."` only when the user can supply a specific official board link or ownership fact; otherwise keep the task with AI. Do not activate or scan it as a confirmed company route. Do not infer failure of a previously verified association from one failed scan.
